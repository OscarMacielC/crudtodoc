...
class ExportarActividadesController extends Controller
{
    public function exportPdf(Request $request)
    {
        ...
        $actividades = $data['query']->with('user')->orderBy('fecha')->get();
    ...
    private function obtenerUsuariosPermitidos(array $data, $usuario, string $rolActual): array
    {
        ...
        if (!empty($data['unidad_administrativa'])) {
            ...
            return User::where(function ($q) use ($data, $gerencias) {
                $q->where('pertenece_id', $data['unidad_administrativa'])->orWhereIn('pertenece_id', $gerencias);
            })->pluck('id')->toArray();
        }
    ...
    private function aplicarFiltrosJerarquia($query, array $data, string $rolActual, array $jerarquia): void
    {
        ...
        if (empty($data['propio']) || $data['propio'] !== '1') {
            $rolesVisibles = collect($jerarquia)->filter(fn($nivel) => $nivel >= $jerarquia[$rolActual])->keys()->toArray();
        ...
        switch ($data['modo_fecha'] ?? null) {
            ...
            case 'personalizado':
                if (!empty($data['fecha_inicio']) && !empty($data['fecha_fin'])) {
                    ...
                    $rangoFechas = 'Del ' . Carbon::parse($data['fecha_inicio'])->translatedFormat('j \d\e F \d\e Y') . ' al ' . Carbon::parse($data['fecha_fin'])->translatedFormat('j \d\e F \d\e Y');
                }
                break;
        }
        return $rangoFechas;
    ...
    {
        $titulo = match ($modoFecha) { 'quincena' => 'Reporte de actividades quincenal', 'mes' => 'Reporte de actividades mensual', 'anual' => 'Reporte de actividades anual', default => 'Reporte de actividades', };
        if ($autorUnico) {
